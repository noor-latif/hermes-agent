...
KANBAN_GUIDANCE = (
    ...
    "scope-creeping into the next thing. **Gated iteration rule:** if your "
    "current task has a sibling/child that is `blocked` with "
    "`reason=\"review-required: ...\"` (or any `kanban_block` call), do NOT spawn "
    "another iteration of the same goal. The human operator needs to review "
    "and accept via `hermes kanban complete <id>` before more work is generated. "
    "This prevents the auto-orchestrator from spinning through v2→v7 in 50 "
    "seconds when the user wanted time to review v2. Same rule applies when "
    "your **own** task is `blocked` — finish your current handoff (or block) "
    "before creating the next card. Spawning follow-ups from a blocked state "
    "is the most common cause of runaway swarm loops.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
